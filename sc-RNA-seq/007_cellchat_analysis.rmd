...
out.dir <- paste0(main.dir, "github/007_cellchat/")
...
colors <- c("#83BED9","#D35E63","#BD2D33","#82B577","#BEDCB0","#A0C893","#3D87AF","#E69498","#DED5E7","#3C1D62",
            "#8A579C","#65307E","#C4AED0","#AB88B9","#633713","#FFBDF2","#A8AB8E",  "#858A61","#5F9E55","#37842E",
            "#65A6C7", "#155F28","#A2D6EC","#D1B695","#EC85D8","#23120A","#217024","#7E1512","#353C20", "#8F6843",
            "#F1B1B4","#B2916E","#D747BB","#B90497","#5A632A","#770262","#1F6A95","#115379","#DB7479")
...
cellchat.CTRL <- readRDS(paste0(main.dir, "github/002_data_objects/cellchat.CTRL.rds"))
cellchat.PAH <- readRDS(paste0(main.dir, "github/002_data_objects/cellchat.PAH.rds"))
# cellchat.CTRL@idents <- factor(cellchat.CTRL@idents, levels= c(Epithelial, Endothelial, Immune,Stroma))
# cellchat.PAH@idents <- factor(cellchat.PAH@idents, levels= c(Epithelial, Endothelial, Immune,Stroma))
# cellchat.CTRL <- updateCellChat(cellchat.CTRL)
# cellchat.PAH <- updateCellChat(cellchat.PAH)
...
ht1 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i], width = 10, height = 14, color.heatmap = "OrRd", color.use = colors)
ht2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i+1], width = 10, height = 14, color.heatmap = "OrRd",color.use = colors)
...
# combining all the identified signaling pathways from different datasets 
pathway.union <- union(object.list[[i]]@netP$pathways, object.list[[i+1]]@netP$pathways)
ht3 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i], width = 10, height = 14, color.heatmap = "OrRd",color.use = colors)
ht4 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i+1], width = 10, height = 14, color.heatmap = "OrRd", color.use = colors)
...
ht2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "all", signaling = pathway.union, title = names(object.list)[i+1], width = 10, height = 14, color.heatmap = "OrRd", color.use = colors)
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
...
targets.use <- c("Lymph","aCap","gCap","Art","V-Pulm","V-Sys")
# Do heatmap based on a merged object
gg1 <- netVisual_heatmap(cellchat, color.use = col.pal$celltype)
gg2 <- netVisual_heatmap(cellchat, color.use = col.pal$celltype, measure = "weight")
p <- gg1 + gg2
...
for(i in 1:length(pathways.show)){
  ...
  netAnalysis_signalingRole_network(cellchat.CTRL, signaling = pathways.show[i],  width = 12,
                                    height = 2.5, font.size = 7, color.use = colors) 
  dev.off()
  }
...
invisible(gc())
dir <- paste(main.dir, "pathway_signaling_roles/PAH/", sep = "/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)}
...
for (i in 1:nrow(mat)) {
  ...
  mat2[i, ] <- mat[i, ]
  p <- netVisual_circle(mat2, vertex.weight = groupize, weight.scale = T, edge.weight.max = max(mat), title.name = rownames(mat)[i], color.use = colors, vertex.label.cex = 0.7)
...
pathway.name <- unique(LR.pairs$pathway_name)
#LR.pairs.sub <- subset(LR.pairs, pathway_name == paste(pathways.show))
# ligands <- unique(LR.pairs.sub$ligand)
# ligands <- unique(unlist(strsplit(x = ligands, "\\_")))
# receptors <- unique(LR.pairs.sub$receptor)
# receptors <- unique(unlist(strsplit(x = receptors, "\\_")))
...
for (pathway in 1:length(pathway.name)) {
  ...
  genes <- rownames(htap[genes,])
  genes <- toupper(genes)
  
  StackedVlnPlot(obj = htap, angle.x = 45,
                 features = genes,
...
invisible(gc())
dir <- paste0(out.dir, "Imm.vs.Endo/LR.pairs/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}

joined <- data.frame()
#sources.use <- c("AM1", "AM2","Mono","Inter-Macro")
sources.use <- c("AM1", "AM2","AM-prolif","DC1","DC2","Mono","Inter-Macro","CD4","CD8","T-prolif","NK","Mast","Plasma","pDC","B","Megak")
targets.use <- c("Lymph","aCap","gCap","Art","V-Pulm","V-Sys")
celltype.to.test <- as.vector(unique(cellchat@idents[[1]]))
celltype.to.test <- celltype.to.test[!celltype.to.test %in% c("PNEC","AM-prolif","T-prolif","B")]
sources.use <- celltype.to.test[celltype.to.test %in% c(sources.use)]
...
for(i in 1:length(sources.use)){
  netVisual <- netVisual_bubble(cellchat,
                                ...
                                color.heatmap = "Spectral",
                                color.text = col.pal$group,
                                title.name = paste("LR pairs -", sources.use[i]),
                                return.data = F
  ) +
  ...
  pdf(paste(dir, sources.use[i], "_LR_pairs.pdf", sep = ""), width = 4, height = 2.5, useDingbats = F)
  print(netVisual)
  dev.off()
  netVisual <- netVisual$data
  rownames(netVisual) <- c()
...
dir <- paste(main.dir, "Imm.vs.Endo/", sep = "/")
LR.pairs <- read.delim(paste0(out.dir, "Imm.vs.Endo/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)

pathways.show <- "CCL"
pairLR.use <- extractEnrichedLR(cellchat, signaling = pathways.show, geneLR.return = F, enriched.only = T)
LR.pairs <- subset(LR.pairs, pathway_name == paste(pathways.show))
sources.use <- unique(LR.pairs$source)
targets.use <- unique(LR.pairs$target)

for(i in 1:length(sources.use)){
  netAnalysis_contribution(cellchat.CTRL, signaling = "BMP")
  ...
  plot <- netVisual_bubble(cellchat,
    ...
    guides(colour = guide_colorbar(frame.colour = "black", ticks.colour = "black")) +
    geom_point(shape = 21, colour="black", stroke=0.5)
  pdf(paste(dir, sources.use[i], "_LR_pairs.pdf", sep = ""), width = 7, height = 3, useDingbats = F)
  print(plot)
  dev.off()
}
...
dir <- paste0(out.dir, "Imm.vs.Endo/pathway_gene_expression/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
...
dir <- paste0(out.dir, "Imm.vs.Endo/signaling_network/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
LR.pairs <- read.delim(paste0(out.dir, "Imm.vs.Endo/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
ranknet <- rankNet(cellchat, mode = "comparison")
...
for (pathway in 1:length(pathway.name)) {
  ...
  weight.max <- getMaxWeight(object.list, slot.name = c("netP"), attribute = pathways.show)
  for (i in 1:length(object.list)) {
    pdf(paste(dir, paste0(pathways.show,"_",names(object.list)[i]),".pdf"), 
...
dir <- paste0(out.dir, "Imm.vs.Endo/LR.pair_contributions/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
LR.pairs <- read.delim(paste0(out.dir, "Imm.vs.Endo/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
ranknet <- rankNet(cellchat, mode = "comparison")
...
invisible(gc())
dir <- paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pairs/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
...
for(i in 1:length(sources.use)){
  ...
  pdf(paste(dir, sources.use[i], "_LR_pairs.pdf", sep = ""), width = 8, height = 4, useDingbats = F)
  print(netVisual)
  dev.off()
  netVisual <- netVisual$data
  rownames(netVisual) <- c()
...
dir <- paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/")
LR.pairs <- read.delim(paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
...
dir <- paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/pathway_gene_expression/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}

Idents(htap) <- "population"
sub <- subset(htap, idents = c("Immune", "Endothelial", "Stroma"))
LR.pairs <- read.delim(paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pairs/LR.pairs.tsv"))
#pairLR.use <- extractEnrichedLR(cellchat.CTRL, signaling = pathways.show, geneLR.return = F)
pathway.name <- unique(LR.pairs$pathway_name)
...
for (pathway in 1:length(pathway.name)) {
  ...
  StackedVlnPlot(obj = sub, 
               angle.x = 45,
               features = genes,
               group.by = "celltype_order", 
               split.by = "group", 
               base.size = 10,
               color.text.x = c(col.pal$endo_colors, col.pal$immune_colors, col.pal$stroma_colors),
               color.use = col.pal$group)
...
dir <- paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/signaling_network/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
LR.pairs <- read.delim(paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
ranknet <- rankNet(cellchat, mode = "comparison")
...
dir <- paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pair_contributions/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = T, showWarnings = F)}
LR.pairs <- read.delim(paste0(out.dir, "Cellchat_Immune_vs_Endo_Stroma/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
ranknet <- rankNet(cellchat, mode = "comparison")
...
colors <- c("#83BED9","#D35E63","#BD2D33","#82B577","#BEDCB0","#A0C893","#3D87AF","#DED5E7","#3C1D62",
            "#8A579C","#65307E","#C4AED0","#AB88B9","#633713","#FFBDF2","#A8AB8E",  "#858A61","#5F9E55","#37842E",
            "#65A6C7", "#155F28","#A2D6EC","#D1B695","#EC85D8","#23120A","#217024","#7E1512","#353C20", "#8F6843",
            "#F1B1B4","#B2916E","#D747BB","#B90497","#E3898E","#5A632A","#770262","#1F6A95","#115379","#DB7479")

invisible(gc())
dir <- paste(main.dir, "Cellchat_Immune_vs_Endo_Stroma/pathway_signaling_roles/CTRL/", sep = "/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)}
...
invisible(gc())
dir <- paste(main.dir, "Cellchat_Immune_vs_Endo_Stroma/pathway_signaling_roles/PAH/", sep = "/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)}
...
for (i in 1:nrow(mat)) {
  ...
  pdf(paste(main.dir, "Cellchat_Immune_vs_Endo_Stroma/celltype_signaling_roles/CTRL/", rownames(mat)[i], "_CTRL.pdf", sep = ""), width = 8, height = 8, useDingbats = FALSE)
  print(p)
  dev.off()
}
...
for (i in 1:nrow(mat)) {
  ...
  p <- netVisual_circle(mat2, 
                        ...
                        color.use = colors, 
                        vertex.label.cex = 0.7)
  pdf(paste(main.dir, "Cellchat_Immune_vs_Endo_Stroma/celltype_signaling_roles/PAH/", rownames(mat)[i], "_PAH.pdf", sep = ""), width = 8, height = 8, useDingbats = FALSE)
  print(p)
  dev.off()
}
