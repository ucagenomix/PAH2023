...
DefaultAssay(integrated) <- "RNA"
integrated@reductions$ref.umap <- NULL
integrated[["refAssay"]] <- NULL
integrated[["prediction.score.ann_level_1"]] <- NULL
integrated[["prediction.score.ann_level_2"]] <- NULL
integrated[["prediction.score.ann_level_3"]] <- NULL
integrated[["prediction.score.ann_level_4"]] <- NULL
integrated[["prediction.score.ann_level_5"]] <- NULL
integrated[["prediction.score.ann_finest_level"]] <- NULL
...
# Highlight each seurat cluster

```{r}

Idents(integrated) <- "seurat_clusters"
seurat_clusters <- as.vector(unique(integrated$seurat_clusters))

for (i in 1:length(seurat_clusters)) {
  celltype <- seurat_clusters[i]
  DimPlot(object = integrated,
          group.by = "seurat_clusters",
          pt.size = 0.001,
          cells.highlight = WhichCells(integrated, idents = celltype),
          cols.highlight = "#1991B4", sizes.highlight = 0.001) +
    ...
    ggtitle(paste("Seurat cluster ", celltype, sep = "")) +
    NoLegend()
  ggsave(paste(out.dir, "/highlight_singlet/", "seurat_cluster_", celltype, ".png", sep = ""), width = 3, height = 3)
...
```{r}
# 
# get_pcs <- function(seurat_object, reduction="pca") {
...
# 
# Idents(integrated) <- "seurat_clusters"
# sub <- subset(integrated, idents = 70, invert = F)
...
# sub <- FindVariableFeatures(sub, assay = "integrated")
# features <- VariableFeatures(sub,  assay = "integrated")
# sub <- ScaleData(sub, assay = "integrated", verbose = T)
# sub <- RunPCA(sub, assay = "integrated",features = features,verbose = T)
# 
# best_pcs <- get_pcs(sub)
# best_pcs <- min(best_pcs)
# 
# sub <- RunUMAP(sub, dims = 1:best_pcs,reduction = "pca", assay = "integrated",verbose = T)
# sub <- FindNeighbors(sub, dims = 1:best_pcs, reduction = "pca", assay = "integrated")
# 
# sub <- FindClusters( object = sub, resolution = "pca", graph.name = "integrated_snn")
# 
...
dim(htap[["RNA"]]@counts)
dim(htap[["RNA"]]@data)
dim(htap[["integrated"]]@counts)
dim(htap[["integrated"]]@data)
```
...
htap@meta.data$seurat_clusters <- htap@meta.data$integrated_snn_res.0.8
Dim_Plot(obj = htap, group.by = "seurat_clusters", label = T, label.ncol = 2) + NoLegend()
...
dim(htap)
#saveRDS(object = htap, paste0(out.dir, "htap.rds"))
...
```{r fig.width=10}
DefaultAssay(htap)<-"RNA"
VlnPlot(htap, group.by = "celltype", features = "CD68")+NoLegend()
```
